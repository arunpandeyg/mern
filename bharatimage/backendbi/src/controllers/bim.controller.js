...
export const getBims = async (req, res) => {
  ...
  const hasNextPage = bims.length === LIMIT;

  // await new Promise((resolve) => setTimeout(resolve, 3000));

  res
    .status(200)
    .json({ bims, nextCursor: hasNextPage ? pageNumber + 1 : null });
};

export const getBim = async (req, res) => {
  const { id } = req.params;
  const bim = await Bim.findById(id).populate(
    "user",
...
export const createBim = async (req, res) => {
  const {
    title,
    description,
    link,
  ...
  let cropbimgStrategy = "";

  if (parsedCanvasOptions.size !== "original") {
    if (originalAspectRatio > clientAspectRatio) {
      cropbimgStrategy = ",cm-pad_resize";
    }
  } else {
    if (
  ...
  const transformationString = `w-${width},h-${height}${cropbimgStrategy},bg-${parsedCanvasOptions.backgroundColor.substring(
    1
  )}${
    parsedTextOptions.text
      ? `,l-text,i-${parsedTextOptions.text},fs-${
  ...
  imagekit
    ...
    .then(async (response) => {
      ...
      return res.status(201).json(newBim);
    })
    .catch((err) => {
      console.log(err);
...
export const interactionCheck = async (req, res) => {
  ...
  const likeCount = await Like.countDocuments({ bim: id });
