  ...
  for (const user of users) {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      const bim = new Bim({
        ...
        title: `Bim ${i} by ${user.username}`,
        description: `This is bim ${i} created by ${user.username}`,
        link: `https://example.com/bim${i}`,
        board: userBoards[i - 1]._id,
        tags: [`tag${i}`, "sample", user.username],
        user: user._id,
      });
      bims.push(await bim.save());
    }
  }

  for (const user of users) {
    for (let i = 1; i <= 10; i++) {
      const randomBim = bims[Math.floor(Math.random() * bims.length)];
      const comment = new Comment({
        description: `Comment ${i} by ${user.username}: This is a great bim!`,
        bim: randomBim._id,
        user: user._id,
      });
      await comment.save();
